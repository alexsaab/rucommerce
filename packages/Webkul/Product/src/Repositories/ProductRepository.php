     ...
     */
        public function searchFromDatabase(array $params = [])
        {
            ...
            $query = $this->with([
                ...
                $qb = $query->distinct()
                    ->select('products.id', 'products.type')
                ...
                if ($sortOptions['order'] != 'rand') {
                    ...
                    if ($attribute) {
                        if ($attribute->code === 'price') {
                            $qb->addSelect('product_price_indices.min_price');
                            $qb->orderBy('product_price_indices.min_price', $sortOptions['order']);
                            $qb->groupBy('products.id', 'products.type', 'product_price_indices.min_price');
                            ...
                            $qb->addSelect($alias.'.'.$attribute->column_name);
                            $qb->orderBy($alias.'.'.$attribute->column_name, $sortOptions['order']);
                            $qb->groupBy('products.id', 'products.type', $alias.'.'.$attribute->column_name);
                        }
                    } else {
                        /* `created_at` is not an attribute so it will be in else case */
                        $qb->orderBy('products.created_at', $sortOptions['order']);
                        $qb->groupBy('products.id', 'products.type');
                    }
                } else {
